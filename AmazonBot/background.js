...
localStorage["botName"]="Amazon Bot";
localStorage['app']="Amazon Bot";
...
function GenerateURL()
{
	if(localStorage['productSearch']=="directLink"){
		...
		var keys ="";
		var colors="";
		try{keys=localStorage['keywords'].replace(/,|_/g,'+')}catch(ex){}
		try{colors=localStorage['color'].replace(",","+")}catch(ex){}
		new_url = "https://www.amazon."+localStorage['site']+"/s?k="+keys;		
...
chrome.extension.onMessage.addListener(
  function(request, sender, sendResponse) {
	...
	if (request.action == "isRecordingOn"){
		...
		werehouse: localStorage['werehouse'],
	...
	if (request.action == "isUserLoged"){
		sendResponse({action: localStorage["uir"]});
	}
