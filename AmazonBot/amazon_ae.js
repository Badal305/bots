	...
	if(response.action == 'true'){
		...
		$(function(){ 
				...
				if($(".s-result-item").is(":visible") && $(".a-price-whole").is(":visible")){					
					...
					try{
						...
						for(var i=0; i<a.length; i++){
							...
							try{var c = a[i].getElementsByClassName("a-offscreen")[0].textContent.trim().replace("AED ","");}catch(err){}
							if(c.match(",")){
								c=c.replace(",","");
							}
							if(KeywordsType=="and"){
			...
			v001 = setInterval(function(){
				if($("#zg-ordered-list").is(":visible")){					
					...
					try{
						...
						for(var i=0; i<a.length ; i++){
							var b = a[i].textContent.trim().replace(/\s{2,}/g, ' ');
							try{var c = a[i].getElementsByClassName("a-color-price")[0].getElementsByTagName("span")[0].textContent.trim().replace("AED ","");}catch(err){}
							if(KeywordsType=="and"){
								if((keysFoundAnd(keywords, b))&&(!findNegativeKeys(keywords2,b) && !b.match('Currently unavailable'))){
			...
			v2=setInterval(function(){
				if(($("#productTitle").is(":visible") && !$("#attach-accessory-pane").is(":visible") && !$("#siAddCoverage-announce").is(":visible") && !$("#attachSiAddCoverage").is(":visible") && !$("#a-popover-header-1:contains('Add to Order')").is(":visible") && (!$("#buybox-see-all-buying-choices-announce").is(":visible") && !$("#buybox-see-all-buying-choices").is(":visible"))) && !$("#aod-offer-list").is(":visible") && ($("#variation_edition").is(":visible") || (!$("#buybox-see-all-buying-choices-announce").is(":visible") || !$("#buybox-see-all-buying-choices").is(":visible")))){
					clearInterval(v2);					
					try{						
								...
								//setTimeout(function(){
									try{
										var b="";
											...
											}else if($("#priceblock_ourprice").is(":visible")){
												...
												for(var k =0; k<stl.length; k++){
													for(var l=0; l<color.length; l++){													
														...
														if(st==color[l]){	
															...
															v00001 = setInterval(function(){
																if($("#priceInsideBuyBox_feature_div").is(":visible")){
																	var sellerDetail=document.getElementById('merchant-info').textContent;
																	if(Seller=="amazon"){
            ...
            if(autocheckoutselect==="yes"){								
				...
				try{
					if(($("#aod-offer-view-cart-1").is(":visible") && document.getElementById("aod-offer-view-cart-1").className !=="a-button a-button-base aod-view-cart-btn aok-hidden") || $(".aod-qty-container").is(":visible")){
						clearInterval(v032);
						location.href="https://www.amazon.ae/gp/cart/view.html?ref_=nav_top_cart";	
					}			
				}catch(err){}	
			},100);
