	...
	if(response.action == 'true'){
		...
		$(function(){ 
			...
			v0012 = setInterval(function(){
				if($("#zg-ordered-list").is(":visible") && $(".a-color-price").is(":visible")){					
					clearInterval(v0012);
					try{
			...
			v2=setInterval(function(){
				if($("#productTitle").is(":visible") && !$("#attach-accessory-pane").is(":visible") && !$("#siAddCoverage-announce").is(":visible") && !$("#siAddCoverage-announce").is(":visible")&& ($("#variation_edition").is(":visible") || !$("#buybox-see-all-buying-choices-announce").is(":visible"))){
					clearInterval(v2);					
					try{						
			...
			v301 = setInterval(function () {
				if($("#all-offers-display").is(":visible")){
					try{
						clearInterval(v301);
          ...
          if(autocheckoutselect==="yes"){	
				vv51 = setInterval(function () {
					if($("h2:contains('Je Amazon-winkelwagen is leeg')").is(":visible")||$("h1:contains('Je Amazon-winkelwagen is leeg')").is(":visible")){
						clearInterval(vv51);
						location.href=url;
					}
				},100); 
				...
				vv53 = setInterval(function () {
					if($("div:contains('There was a problem with')").is(":visible")){
						clearInterval(vv53);
						location.href="https://www.amazon.nl/gp/cart/view.html?ref_=nav_cart";					
					}
				},1000);				
				...
				v56 = setInterval(function () {
					if($("#sw-atc-actions").is(":visible")){
						clearInterval(v56);
						try{
							document.getElementById("sw-atc-actions").getElementsByClassName("a-button-text")[0].click();
						}catch(err){}						
					}
				},10);	
