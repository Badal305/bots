		...
		$(function(){ 
				...
				if($(".s-result-item").is(":visible") && $(".a-price-whole").is(":visible")){					
					...
					try{
						...
						for(var i=0; i<a.length ; i++){
							...
							try{var c = a[i].getElementsByClassName("a-price-whole")[0].textContent.trim().split(",");}catch(err){}
							if(KeywordsType=="and"){
						...
						for(var i=0; i<a.length ; i++){
							...
							if(KeywordsType=="and"){
								if((keysFoundAnd(keywords, b))){
									...
									try{var p= prc.match(/(\d+)/g);}catch(ree){}
													...
													if(s.options[i].textContent.trim().toLowerCase()==size[j]){
														...
														v01 = setInterval(function(){
															if($("#priceInsideBuyBox_feature_div").is(":visible")){
																...
																if(Seller=="amazon"){												
																	if(sellerDetail.match("Vendido y enviado por Amazon.")){
																		ATC();
																	}
																}else{
																	ATC();
            ...
            if(autocheckoutselect==="yes"){	
				vv556 = setInterval(function () {
					if($("h1:contains('Tu cesta de Amazon está vacía')").is(":visible") ||$("h2:contains('Tu cesta de Amazon está vacía')").is(":visible")){
						clearInterval(vv556);
						location.href=url;				
					}
				},100);
				...
				v032 = setInterval(function () {
					try{
						if(($("#aod-offer-view-cart-1").is(":visible") && document.getElementById("aod-offer-view-cart-1").className !=="a-button a-button-base aod-view-cart-btn aok-hidden") || $(".aod-qty-container").is(":visible")){
							clearInterval(v032);
							location.href="https://www.amazon.es/gp/cart/view.html?ref_=nav_cart";	
						}			
					}catch(err){}	
				},100);
