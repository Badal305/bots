function save()
{
	...
	localStorage['werehouse'] = $("#werehouse").val();
...
$(function(){
	...
	if(localStorage.getItem("werehouse") === null) {
		$("#werehouse").val();
	}else{
		$("#werehouse").val(localStorage['werehouse']);
	}
	...
	if (localStorage.getItem("AccountBilling") == "true") {
		try{document.getElementById("AccountBilling").checked=true;}catch(err){}
	}
	...
	//Show hide button 	
try{
	function togglePasswordfunc(butn,passwordfield){
	...
	try{
		const togglePassword1 = document.getElementsByClassName('togglePassword1')[0];
		const password1 = document.getElementsByName("username2")[0];
			togglePasswordfunc(togglePassword1,password1);

		const togglePassword2 = document.getElementsByClassName('togglePassword2')[0];
		const password2 = document.getElementsByName("password2")[0];
		togglePasswordfunc(togglePassword2,password2);

		const togglePassword3 = document.getElementsByClassName('togglePassword3')[0];
		const password3 = document.getElementsByName("payMethodPaneCardNumber")[0];
		togglePasswordfunc(togglePassword3,password3);

		const togglePassword4 = document.getElementsByClassName('togglePassword4')[0];
		const password4 = document.getElementsByName("payMethodPaneCVV")[0];
		togglePasswordfunc(togglePassword4,password4);

		const togglePassword5 = document.getElementsByClassName('togglePassword5')[0];
		const password5 = document.getElementsByName("paypalEmail")[0];
		togglePasswordfunc(togglePassword5,password5);

		const togglePassword6 = document.getElementsByClassName('togglePassword6')[0];
		const password6 = document.getElementsByName("paypalPassword")[0];
		togglePasswordfunc(togglePassword6,password6);
	}catch(err){}
});
...
function displayOtpArea()
{
	$("#loginForm .clear").html('<img src="img/loading.gif" style="height: 20px; -webkit-filter: sepia(100%) hue-rotate(67deg) invert(13%);">');
	$(".mainContainer").html('<div align="center"><input type="text" placeholder="ENTER OTP" id="orderOtp" class="form-control" name="orderOtp" style="color:black"><br/><input type="submit" style="color:white;background-color:#c00a0a" class="form-control" id="validateOtp" value="VERYFIE OTP"><br/><div style:"display:inline"><p>NOT RECIEVED OTP ? <a style="color:#56da13" id="resendOtp">RSEND OTP</a></p></div> </div> <br/>');	
}
function Authorize(){	
	$.ajax({
		type: "POST",
		dataType: "json",
		data: {'orderEmail': localStorage['orderEmail'], botName: localStorage["botName"], orderId:localStorage['orderId']},
		url: "https://ebotlab.com/otp99/check.php"
	}).done(function(res){
		if(res.status=="true"){
			localStorage["otp"]=res.otp;
			localStorage["otpWait"]='true';	
			displayOtpArea();
		}else if(res.status=="false"){
			alert("Invalid Data! Either your Email Id or Order Id is not matched");
		}else{
...
function AuthorizeOtp(){	
	$.ajax({
		type: "POSt",
		data: {'orderEmail': localStorage['orderEmail'], botName: localStorage["botName"], orderId:localStorage['orderId'],orderOtp:localStorage['orderOtp']},
		url: "https://ebotlab.com/otp99/check.php"
	}).success(function(res){
		if(res.trim()=="true")
		{
			localStorage["otpWait"]=false;
			localStorage["uir"]=true;
			location.reload();
		}else{
			alert("Invalid OTP!");
		}
...
$("#validate").live("click", function(){
	localStorage['orderEmail'] = $("#orderEmail").val();
	localStorage['orderId'] = $("#orderId").val();
	Authorize();
});
$("#validateOtp").live("click", function(){
	localStorage['orderOtp'] = $("#orderOtp").val();	
	AuthorizeOtp();
	
});
$("#resendOtp").live("click", function(){
	Authorize();
	alert("Otp resent. Please check your email");
});
v90=setInterval(function(){
	if($("#orderId").is(":visible"))
	{
		$("#orderId").val(localStorage['orderId']);
		$("#orderEmail").val(localStorage['orderEmail']);
		clearInterval(v90);
	}
},100);
v91=setInterval(function(){
	if($("#orderOtp").is(":visible"))
	{
		//$("#orderOtp").val(localStorage['otp']);
		clearInterval(v91);
		alert("Please check your email '"+ localStorage['orderEmail']+ "' inbox or spam folder to get the OTP.");		
	}
},100);
chrome.extension.sendMessage({action: "isUserLoged"}, function(response) {
	if(response.action== 'false'){
		var myhtml = "";
		//alert(localStorage['otpWait']);
		if(localStorage['otpWait']=='true')
		{
			displayOtpArea();			
		}else{
			myhtml = '<div id="loginForm"><h4>Email Id: </h4><input type="text" id="orderEmail" placeholder="Purchased Email" name="orderEmail"class="form-control" ><h4>Order Id: </h4><input type="text" id="orderId" placeholder="Order Id " name="orderId" class="form-control" ><div><center> <br/><div id="validate"><input type="button"  class="form-control" style="background-color:green;color:white" value="OK"></button></div><div class="clear" style="padding-top:10px; padding-bottom:10px;"></div><br/></div></div>';
			$(".mainContainer").html('<div class="buttonContainer">'+myhtml+'</div>');
			/*$("#signIn").live("click", function(){
				$("#loginForm").removeClass("hidden");
				$("#login_email").focus();
			});*/
...
$('#deactivate').live('click', function(){ 
	$.ajax({
	  type: "POST",
	  url: "http://mostadvancedbot.com/activations/deactivate.php",
	  data: {'app': localStorage['app'], 'email': localStorage['userEmail'], 'action': 'delete'}
	}).done(function( res ) {
        if(res == "success"){
			chrome.extension.sendMessage({action: "deactivate"}, function(response) {
	...
	$.ajax({
	  ...
	  data: {'email': localStorage['userEmail']}
        ...
        if(res == "true"){
            localStorage['userEmail']="";
            chrome.extension.sendMessage({action: "logout"}, function(response) { });     
            signInHTML = '<div id="loginForm" class="hidden"><table ><tr><td align="right"><h4>Email Id: </h4></td><td align="left" style="padding-left:10px; padding-right:10px;"><input type="text" id="login_email" placeholder="Purchased Email" name="login_email" class="form-control" ></td></tr><tr><td align="right"><h4>Order Id: </h4></td><td align="left" style="padding-left:10px; padding-right:10px;"><input type="text" id="order_id" placeholder="Order Id " name="order_id" class="form-control" ></td></tr></table><div><center> <div id="validate"><button  class="btn btn-success "><b>OK</b></button></div><div class="clear" style="padding-top:10px; padding-bottom:10px;"></div><br/></div></div>';   
            $(".mainContainer").html('<div class="buttonContainer">'+signInHTML+'</div>');
            /*$("#signIn").live("click", function(){
                $("#loginForm").removeClass("hidden");
                $("#login_email").focus();
            ...
            */
            $("#validate").live("click", function(){
                localStorage['userEmail'] = $("#login_email").val();
				localStorage['order_id'] = $("#order_id").val();
                $("#loginForm .clear").html('<img src="img/loading.gif" style="height: 20px; -webkit-filter: sepia(100%) hue-rotate(67deg) invert(13%);">');
                chrome.extension.sendMessage({action: "CheckUserRegistered"}, function(response) {
                    setTimeout(function(){
                        chrome.extension.sendMessage({action: "isUserLoged"}, function(response) {
