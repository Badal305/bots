	...
	if(response.action == 'true'){
		...
		$(function(){ 
			...
			v02 = setInterval(function(){
				if($("li[class*='style__itemOuter']").is(":visible") && $(".price").is(":visible")){					
					clearInterval(v02);				
					try{
			...
			v030 = setInterval(function(){
				if($("div[class*='style__showcase']").is(":visible") && !$(".price").is(":visible")){
					clearInterval(v030);				
					try{						
						var a = $("div[class*='style__showcase']");
						for(var i=0; i<a.length; i++){
							var b = a[i].textContent.trim();							
							for(var j=0; j<keywords.length; j++){
								if(KeywordsType=="and"){
									if((keysFoundAnd(keywords, b))&&(!findNegativeKeys(keywords2,b) && !b.match('Actuellement indisponiblee'))){
										location.href=a[i].getElementsByTagName('a')[0].href;
										return;
									}						
								}else{
									if((keysFoundOr(keywords, b))&&(!findNegativeKeys(keywords2,b) && !b.match('Actuellement indisponiblee'))){			
										location.href=a[i].getElementsByTagName('a')[0].href;
										return;									
									}									
								}								
				...
				if($(".apb-browse-col-pad-left").is(":visible") && $(".a-price-whole").is(":visible")){	
					...
					try{
						...
						for(var i=0; i<a.length ; i++){
							var b = a[i].textContent.trim().replace(/\s{2,}/g, ' ');
							try{
								var p = a[i].getElementsByClassName("a-price-whole")[0].textContent.trim().split(",");
							}catch(err){}
							var c=p[0].trim();
							if(KeywordsType=="and"){
			...
			v2=setInterval(function(){
				if($("#productTitle").is(":visible") && !$("#attach-accessory-pane").is(":visible") && !$("span:contains('Non, merci')").is(":visible") && !$("#siAddCoverage-announce").is(":visible") && !$("#siAddCoverage-announce").is(":visible") && ($("#variation_edition").is(":visible") || !$("#buybox-see-all-buying-choices-announce").is(":visible")) && (!$("#buybox-see-all-buying-choices-announce").is(":visible") && !$("#buybox-see-all-buying-choices").is(":visible")) && !$("span:contains('Non, merci')").is(":visible")){
					clearInterval(v2);					
					try{
													...
													if(s.options[i].textContent.trim().toLowerCase()==size[j]){
														...
														v01 = setInterval(function(){
															if($("#priceInsideBuyBox_feature_div").is(":visible")){
																...
																if(Seller=="amazon"){
																	if(sellerDetail.match("Expédié et vendu par Amazon.")){
																		ATC();
																	}
																}else{
																	ATC();
			...
			v033 = setInterval(function () {
				try{
					if($("#aod-offer-qty-button-1-announce").is(":visible") || $("#aod-offer-view-cart-1-announce").is(':visible')){
						clearInterval(v033);
						location.href="https://www.amazon.fr/gp/cart/view.html?ref_=nav_cart";			
			...
			v34 = setInterval(function () {
				try{
				if($("span:contains('Non, merci')").is(":visible") && document.getElementsByClassName("a-button a-button-base abb-intl-decline")[0].getElementsByTagName("input")[0]){
					clearInterval(v34);
            ...
            if(autocheckoutselect==="yes"){	
				vv51 = setInterval(function () {
					if($("h2:contains('Votre panier Amazon est vide')").is(":visible")||$("h1:contains('Votre panier Amazon est vide')").is(":visible")){
						clearInterval(vv51);
						location.href=url;
					}
				},100); 
				...
				vv556 = setInterval(function () {
					if($("td:contains('Une erreur de système interne a été décelée. Nos équipes techniques se chargent de la résoudre. Nous vous prions de nous excuser pour la gêne causée')").is(":visible")){
						clearInterval(vv556);
						location.href=url;				
					}
				},100);
				...
				v67 = setInterval(function(){
					if($("#sw-atc-actions").is(":visible") && $("#a-autoid-0-announce").is(":visible")){
						clearInterval(v67);
						try{
							if(soundAlert=="yes"){
								chrome.extension.sendMessage({action: "alert"}, function(response) {});
