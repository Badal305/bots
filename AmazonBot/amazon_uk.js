	...
	if(response.action == 'true'){
		...
		$(function(){ 
						...
						for(var i=0; i<a.length; i++){
							...
							if(KeywordsType=="and"){
								if((keysFoundAnd(keywords, b))&&(!findNegativeKeys(keywords2,b) && !b.match('Currently unavailable'))){
									if((low<=Number(c) && high>=Number(c)) || (low==0 && high==0)){
			...
			v2=setInterval(function(){
				if(($("#productTitle").is(":visible") && !$("#attach-accessory-pane").is(":visible") && !$("#siAddCoverage-announce").is(":visible") && !$("#a-popover-1").is(":visible"))&& !$("#aod-offer-list").is(":visible")&& ($("#variation_edition").is(":visible") || (!$("#buybox-see-all-buying-choices-announce").is(":visible") && !$("#buybox-see-all-buying-choices").is(":visible")))){
					clearInterval(v2);
				//	setTimeout(function(){
						try{	
											...
											if($("#native_dropdown_selected_size_name").is(":visible")){
												...
												try{var sellerDetail=document.getElementById('merchant-info').textContent;}catch(er){}
													try{if(Seller=="amazon"){												
														if(sellerDetail.match("sold by Amazon")){
															ATC();
														}
													}else{
														ATC();
			...
			v4 = setInterval(function () {
				if($("#siAddCoverage-announce").is(":visible")){										
					if(warranty=="yes"){
						document.getElementById("siAddCoverage").getElementsByTagName("button")[0].click();
			...
			v031 = setInterval(function () {
				if(($("#buybox-see-all-buying-choices-announce").is(":visible")|| $("#buybox-see-all-buying-choices").is(":visible")) && (!$("#add-to-cart-button").is(":visible")||!$("#add-to-cart-button-ubb").is(":visible")) && (($("#variation_edition").is(":visible") && $("#variation_platform_for_display").is(":visible")) || !$("#variation_edition").is(":visible") || $("#variation_platform_for_display").is(":visible"))){
					try{
						clearInterval(v031);
						try{document.getElementById("buybox-see-all-buying-choices-announce").click();}catch(err){}
						try{document.getElementById("buybox-see-all-buying-choices").getElementsByTagName("a")[0].click();}catch(err){}
            ...
            if(autocheckoutselect==="yes"){	
				...
				vv56 = setInterval(function () {
					if($("b:contains('An error occurred when we tried to process your request.')").is(":visible")){
						clearInterval(vv56);
						location.href=url;				
					}
				},100);
