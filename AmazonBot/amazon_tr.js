...
chrome.extension.sendMessage({action: "isRecordingOn"}, function(response){
	if(response.action == 'true'){
		...
		$(function(){ 
					...
					clearInterval(v0);
						try{
							...
							for(var i=0; i<a.length ; i++){
								...
								try{var p=c.replace(" TL","");}catch(err){}
								if(p.match(",",".")){
									r=p.replace(",",".");
								}
								if(KeywordsType=="and"){
			...
			v010 = setInterval(function(){
				if($(".s-card-container").is(":visible") && !$(".a-price-whole").is(":visible")){
					clearInterval(v010);
						try{
							var a = document.getElementsByClassName("s-card-container");
							for(var i=0; i<a.length ; i++){
								var b = a[i].textContent.trim().replace(/\s{2,}/g, ' ');
								if(KeywordsType=="and"){
									if((keysFoundAnd(keywords, b))&&(!findNegativeKeys(keywords2,b) && !b.match('Currently unavailable'))){
			...
			v2=setInterval(function(){
				if(($("#productTitle").is(":visible") && !$("#attach-accessory-pane").is(":visible") && !$("#siAddCoverage-announce").is(":visible") && !$("#siAddCoverage-announce").is(":visible") && !$("#a-popover-header-1:contains('Add to Order')").is(":visible")) && !$("#aod-offer-list").is(":visible")&& (!$("#buybox-see-all-buying-choices-announce").is(":visible") && !$("#buybox-see-all-buying-choices").is(":visible")) ){
					clearInterval(v2);					
					try{						
			...
			v032 = setInterval(function () {
				try{
					if($("#aod-offer-qty-button-1-announce").is(":visible") || $("#aod-offer-view-cart-1-announce").is(':visible')){
						clearInterval(v032);
						location.href="https://www.amazon.com.tr/gp/cart/view.html?ref_=nav_cart";
			...
			*/
            ...
            if(autocheckoutselect==="yes"){
				vv51 = setInterval(function () {
					if($("h2:contains('Amazon Sepetiniz boş')").is(":visible")||$("h1:contains('Amazon Sepetiniz boş')").is(":visible")){
						clearInterval(vv51);
						location.href=url;
					}
				},100);	
