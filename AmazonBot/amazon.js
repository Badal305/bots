...
chrome.extension.sendMessage({action: "isRecordingOn"}, function(response){
	if(response.action == 'true' && response.site == 'com'){
		...
		var	werehouse = response.werehouse;
		...
		$(function(){ 
			...
			v0 = setInterval(function(){
				if($(".s-result-item").is(":visible") && $(".a-price-whole").is(":visible")){					
					...
					try{
						var c="";
						var s="";
						try{var a = document.getElementsByClassName("s-result-item s-asin");}catch(er){}
						for(var i=0; i<a.length; i++){
							var b = a[i].textContent.trim().replace(/\s{2,}/g, ' ');
							try{var s=a[i].getElementsByClassName("sg-row")[0].textContent;}catch(err){}
							if(KeywordsType=="and"){
								if((keysFoundAnd(keywords, b))&&(!findNegativeKeys(keywords2,b)) && (!b.match('Currently unavailable')  && s.match("$"))){									
									try{var c = a[i].getElementsByClassName("a-offscreen")[0].textContent.trim().replace("$","");}catch(err){}
									if(c.match(",")){
										c=c.replace(",","");
									}
									if((low<=Number(c) && high>=Number(c)) || (low==0 && high==0) || Number(c)==0){
										a[i].getElementsByClassName('a-text-normal')[0].click();
										return;
									}
								}						
							}else{
								if((keysFoundOr(keywords, b))&&(!findNegativeKeys(keywords2,b)) && (!b.match('Currently unavailable') && s.match("$"))){
									try{var c = a[i].getElementsByClassName("a-offscreen")[0].textContent.trim().replace("$","");}catch(err){}
									if(c.match(",")){
										c=c.replace(",","");
									}
			...
			v00 = setInterval(function(){
				if($("[data-card-metrics-id='octopus-search-result-card_apb-browse_0']").is(":visible")){
					clearInterval(v00);
					try{
						try{var a = document.getElementsByClassName("s-expand-height s-include-content-margin s-latency-cf-section");}catch(er){}
						for(var i=0; i<a.length; i++){
							var b = a[i].textContent.trim().replace(/\s{2,}/g, ' ');
							try{var s=a[i].getElementsByClassName("sg-row")[0].textContent;}catch(err){}
							if(KeywordsType=="and"){
			...
			v002 = setInterval(function(){
				if($(".stores-row").is(":visible")){					
					clearInterval(v002);
					try{
						var a = $("li[class*='ProductGridItem__itemOuter']");
						for(var i=0; i<a.length ; i++){
							var b = a[i].textContent.trim().replace(/\s{2,}/g, ' ');
							if(KeywordsType=="and"){
								...
								if((keysFoundOr(keywords, b))&&(!findNegativeKeys(keywords2,b)) && !b.match('Currently unavailable')){									
									a[i].getElementsByTagName('a')[0].click();
									return;									
								}									
							}
			...
			v004 = setInterval(function(){
				if($("#gridItemRoot").is(":visible")){					
					clearInterval(v004);
					try{
						var a = document.getElementsByClassName("zg-grid-general-faceout");
						for(var i=0; i<a.length ; i++){
							var b = a[i].textContent.trim().replace(/\s{2,}/g, ' ');
							if(KeywordsType=="and"){
								if((keysFoundAnd(keywords, b))){
				...
				if($("#aod-offer-list").is(":visible")){
					try{
						...
						for(var i=0; i<a.length;i++){
							...
							var p = b.split(".");
							var r= p[0].match(/(\d+)/g)+"."+p[1].match(/(\d+)/g);	
							if(r.match(",")){
								r=r.replace(",","");
							}
							if((low<=Number(r) && high>=Number(r)) || (low==0 && high==0)){
								var b =a[i].getElementsByTagName("input");
								for(j=0; j<b.length;j++)
								{									
									if(b[j].name=="submit.addToCart"){ b[j].click();return}
			...
			v032 = setInterval(function () {
				try{
					if(($("#aod-offer-view-cart-1").is(":visible") && document.getElementById("aod-offer-view-cart-1").className !=="a-button a-button-base aod-view-cart-btn aok-hidden") || $(".aod-qty-container").is(":visible")){
						clearInterval(v032);
			...
			/*
			v34 = setInterval(function () {
				if($("#a-autoid-21-announce").is(":visible")){
					
					try{
						setTimeout(function(){
							document.getElementById("a-autoid-21-announce").click();							
						},500);
						clearInterval(v34);
					}catch(err){}						
				}
			...
			*/
			...
			v44 = setInterval(function () {
				if($("#a-popover-header-1:contains('Add to Order')").is(":visible")){
					clearInterval(v44);
					try{
						if(warranty=="yes"){
							$("#a-autoid-20-announce").click();
						}else{
							document.getElementsByClassName(" a-button-close a-declarative")[0].click();
						}
					}catch(err){}						
				}
			},3000); 
			v445 = setInterval(function () {
				if($("#a-popover-content-1").is(":visible") && $("[type='checkbox']").is(":visible")){
					try{
						clearInterval(v445);
						if(warranty=="yes"){
							document.getElementsByClassName("abb-intl-product-ato")[0].getElementsByTagName("input")[0].click();
							document.getElementsByClassName("abb-intl-product-ato")[0].getElementsByTagName("input")[0].dispatchEvent(new Event('click', { bubbles: true }));
							//document.getElementsByClassName("abb-intl-product-ato")[0].getElementsByTagName("input")[0].checked=true;
							v446 = setInterval(function () {
								if($("#a-popover-content-1").is(":visible")){ 
									clearInterval(v446);
									//$("[aria-labelledby='a-autoid-29-announce']").click();
									document.getElementById("a-autoid-28-announce").click();
								}
							},300);
						}else{
							document.getElementById("a-autoid-29-announce").click();						
						}
					}catch(err){}						
				}
			},2000); 
            if(autocheckoutselect==="yes"){								
				...
				vv556 = setInterval(function () {
					if($("h1:contains('Your Amazon Cart is empty')").is(":visible") ||$("h2:contains('Your Amazon Cart is empty')").is(":visible")){
						clearInterval(vv556);
						location.href=url;				
					}
				},100);
				vv56 = setInterval(function () {
					if($("div:contains('Some items are no longer available Were sorry')").is(":visible")){
						clearInterval(vv56);
						location.href=url;				
					}
				},100);
				vv57 = setInterval(function () {
					if($("#a-autoid-0-announce:contains('Sign in securely')").is(":visible")){
						clearInterval(vv57);
						if(soundAlert=="yes"){
							chrome.extension.sendMessage({action: "alert"}, function(response) {});
						}
						document.getElementsByClassName("sw-atc-signin")[0].getElementsByTagName("a")[0].click();			
					}
				},100);
               ...
               v12 = setInterval(function(){ 
                    try{
						if(use_address=="yes"){		
							if($(".ship-to-this-address").is(":visible")||$("a:contains('Use this address')").is(":visible") || $("a:contains('Deliver to this address')").is(":visible"))
							{
								clearInterval(v12);
							...
							try{
								if($("[value='Use this address']").is(":visible") ){								
									setTimeout(function(){
										clearInterval(v12);
							...
							if($("#shipToThisAddressButton-announce").is(":visible")||$("span:contains('Use this address')").is(":visible"))
							{
								clearInterval(v12);
								try{$("#shipToThisAddressButton-announce").click()}catch(err){}
								try{$("span:contains('Use this address')").click()}catch(err){}
                    ...
                    try{                    
                        if($(".pmts-select-payment-instrument-form").is(":visible")){
							...
							if(paymentMethod=="amazonStoreCard"){
								...
								document.getElementsByClassName("pmts-add-cc-default-trigger-link")[0].click();
								//$("[aria-labelledby='orderSummaryPrimaryActionBtn-announce']").click();
							}
                        }                               
                   }catch(err){}
				...
				v2001 = setInterval(function () {
					try{
						if($("[value='Place your order']").is(":visible") ){								
							setTimeout(function(){
								$("[value='Place your order']").click();
							},1000);
							clearInterval(v2001);
						}
					}catch(err){}
				},500);
				v2002 = setInterval(function () {
					try{
						if($("#orderSummaryPrimaryActionBtn-announce").is(":visible")||$("span:contains('Use this payment method')").is(":visible"))
							{
								clearInterval(v2002);
								try{$("span:contains('Use this payment method')").click()}catch(err){}
								try{document.getElementById("orderSummaryPrimaryActionBtn-announce").click();}catch(err){}
							}	
					}catch(err){}
				},500);
				v21 = setInterval(function(){
					try{
						if(document.getElementsByTagName("title")[0].textContent=="Sorry! Something went wrong!"){
							clearInterval(v21);
							setTimeout(function(){
								location.reload();
							},getRndRefreshRange(timeoutMin, timeoutMax)); 				
				...
				v23 = setInterval(function () {
					if($("body").is(":visible")&& !$("#nav-logo-sprites").is(":visible")){
						clearInterval(v23);
						setTimeout(function(){
							location.reload();
						},getRndRefreshRange(timeoutMin, timeoutMax)); 
				...
				try{
					v24 = setInterval(function () {
						if(!$("body").is(":visible")&& document.URL!==""){
							clearInterval(v24);
							setTimeout(function(){
								location.reload();
							},getRndRefreshRange(timeoutMin, timeoutMax)); 
				...
				v25 = setInterval(function () {
					try{
						if($("div:contains('Some items are no longer available')").is(":visible")){
							clearInterval(v25);
							location.href=url;		
						}
					}catch(err){}	
