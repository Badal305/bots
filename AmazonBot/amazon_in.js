...
chrome.extension.sendMessage({action: "isRecordingOn"}, function(response) {
	if((response.action == 'true')&&(response.site == 'in')){
		...
		$(function(){
           ...
           v0 = setInterval(function(){
				if($("#zg-ordered-list").is(":visible") && $(".p13n-sc-price").is(":visible")){					
					clearInterval(v0);
					try{
						var s="";
						var c="";
						var a = document.getElementById("zg-ordered-list").getElementsByTagName("li");
						for(var i=0; i<a.length ; i++){
							var b = a[i].textContent.trim().replace(/\s{2,}/g, ' ');
							try{var s=a[i].getElementsByClassName("sg-row")[0].textContent;}catch(err){}
							try{var c = a[i].getElementsByClassName("p13n-sc-price")[0].textContent.trim().replace("₹","");}catch(err){}						
							if(KeywordsType=="and"){
			...
			v020 = setInterval(function(){
				if($(".s-result-item").is(":visible") && $(".a-price-whole").is(":visible")){					
					...
					try{
						...
						for(var i=0; i<a.length; i++){
							try{var b = a[i].textContent.trim().replace(/\s{2,}/g, ' ');}catch(ree){}
							try{ s=a[i].getElementsByClassName("sg-row")[0].textContent;}catch(ree){}
							if(KeywordsType=="and"){
			...
			v2=setInterval(function(){
				if(($("#productTitle").is(":visible") && !$("#attach-accessory-pane").is(":visible") && !$("#siAddCoverage-announce").is(":visible") && !$("#siAddCoverage-announce").is(":visible") && !$("#a-popover-header-1:contains('Add to Order')").is(":visible")) && !$("#aod-offer-list").is(":visible") && ($("#variation_edition").is(":visible") || (!$("#buybox-see-all-buying-choices-announce").is(":visible") || !$("#buybox-see-all-buying-choices").is(":visible"))) && !$("#attach-warranty").is(":visible")){
					clearInterval(v2);					
					try{						
