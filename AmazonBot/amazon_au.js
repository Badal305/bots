			...
			v2=setInterval(function(){
				if(($("#productTitle").is(":visible") && !$("#attach-accessory-pane").is(":visible") && !$("#siAddCoverage-announce").is(":visible") && !$("#siAddCoverage-announce").is(":visible") && !$("#a-popover-header-1:contains('Add to Order')").is(":visible")) && !$("#aod-offer-list").is(":visible") && ($("#variation_edition").is(":visible") || (!$("#buybox-see-all-buying-choices-announce").is(":visible") || !$("#buybox-see-all-buying-choices").is(":visible")))){
					...
					try{						
						...
						if($(".a-color-price").is(":visible") || !a.match("Currently unavailable")){
							if($("#add-to-cart-button").is(":visible") || $("#add-to-cart-button-ubb").is(":visible") || $("#one-click-button").is(":visible") || $("#variation_edition").is(":visible")){
								//setTimeout(function(){
									try{
										var b="0,0 ";									
									...
									if((low<=r && high>=r) || (low==0 && high==0) || (r==0)){
												...
												for(var j=0; j<size.length; j++){
													if(s.options[i].textContent.trim().toLowerCase()==size[j]){
														...
														v01 = setInterval(function(){
															if($("#priceInsideBuyBox_feature_div").is(":visible") || $("#corePriceDisplay_desktop_feature_div").is(":visible")){
										...
										}else if($("#variation_edition").is(":visible")){
            ...
            if(autocheckoutselect==="yes"){								
				...
				vv53 = setInterval(function () {
					if($("div:contains('There was a problem with')").is(":visible")){
						clearInterval(vv53);
						location.href="https://www.amazon.com.au/gp/cart/view.html?ref_=nav_cart";					
					}
				},1000);				
