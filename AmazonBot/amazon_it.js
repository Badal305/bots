...
chrome.extension.sendMessage({action: "isRecordingOn"}, function(response){
	if(response.action == 'true'){
		...
		$(function(){ 
			...
			v0121 = setInterval(function(){
				if($(".padCenterContainer").is(":visible")){	
					clearInterval(v0121);
					try{
						var a = document.getElementsByClassName("padCenterContainer")[0].getElementsByClassName("tallCellView");
						for(var i=0; i<a.length; i++){
							var b = a[i].textContent.trim().replace(/\s{2,}/g, ' ');
							if(KeywordsType=="and"){
			...
			v012 = setInterval(function(){
				if($("[data-csa-c-painter='octopus-search-result-card-cards']").is(":visible")){
					clearInterval(v012);
					try{
						var a = document.getElementsByClassName("s-include-content-margin");
						for(var i=0; i<a.length ; i++){
							var b = a[i].textContent.trim().replace(/\s{2,}/g, ' ');
							...
							if(KeywordsType=="and"){
								if((keysFoundAnd(keywords, b))&&(!findNegativeKeys(keywords2,b) && !b.match('Currently unavailable'))){
									if((low<=c && high>=c) || (low==0 && high==0)){
										a[i].getElementsByClassName('a-link-normal a-text-normal')[0].click();
										return;
									}
								}						
							}else{
			...
			v2=setInterval(function(){
				if(($("#productTitle").is(":visible") && !$("#attach-accessory-pane").is(":visible") && !$("#siAddCoverage-announce").is(":visible") && !$("#a-popover-header-1:contains('Add to Order')").is(":visible") &&(!$("#buybox-see-all-buying-choices-announce").is(":visible") && !$("#buybox-see-all-buying-choices").is(":visible"))) && !$("#attach-warranty").is(":visible") && !$("#attach-added-to-cart-alert-and-image-area").is(":visible")){
					clearInterval(v2);	
					try{						
								...
								//setTimeout(function(){
									...
									if((low<=r && high>=r) || (low==0 && high==0)){
										...
										if($("#native_dropdown_selected_size_name").is(":visible")){
													...
													if(s.options[i].textContent.trim().toLowerCase()==size[j]){
														...
														v01 = setInterval(function(){
															if($("#priceInsideBuyBox_feature_div").is(":visible")){
																...
																if(Seller=="Venduto e spedito da Amazon"){
											...
											try{var sellerDetail=document.getElementsByClassName('tabular-buybox-text a-spacing-none')[1].textContent.trim();}catch(er){}
											if(Seller=="amazon"){
												if(sellerDetail.match("Amazon")){
													ATC();
													return;
			...
			v031 = setInterval(function () {
					if(($("#buybox-see-all-buying-choices-announce").is(":visible") || $("#buybox-see-all-buying-choices").is(":visible")) && (((!$("#add-to-cart-button").is(":visible")||!$("#add-to-cart-button-ubb").is(":visible")) && !$("#variation_edition").is(":visible")) || ($("#variation_edition").is(":visible") && $("#variation_platform_for_display").is(":visible")))){
					try{
						clearInterval(v031);
						try{document.getElementById("buybox-see-all-buying-choices-announce").click();}catch(err){}
						try{document.getElementById("buybox-see-all-buying-choices").getElementsByTagName("a")[0].click();}catch(err){}
			...
			v032 = setInterval(function () {
				try{
					if($("#aod-offer-qty-button-1-announce").is(":visible") || $("#aod-offer-view-cart-1-announce").is(':visible')){
						clearInterval(v032);
						location.href="https://www.amazon.it/gp/cart/view.html?ref_=nav_cart";
									
					}			
				}catch(err){}	
			},100);
			v033 = setInterval(function () {
				try{
				if($("#aod-offer-qty-button-1-announce").is(":visible") || $("#aod-offer-view-cart-1-announce").is(':visible') || $("#sw-atc-actions").is(':visible') ){
						clearInterval(v033);
						location.href="https://www.amazon.it/gp/cart/view.html?ref_=nav_cart";
			...
			v03 = setInterval(function () {
				if($("#all-offers-display").is(":visible") && $(".a-price-whole").is(":visible") && $(".aod-information-block").is(":visible")){
					try{
						clearInterval(v03);
						var a=document.getElementsByClassName("aod-information-block");
						for(var i=1; i<a.length;i++){
							var b=a[i].getElementsByClassName("a-price-whole")[0].textContent.trim();
							try{var r= b.trim().replace(".","");}catch(err){}
							var p= r.trim().replace(",","");
							if((low<=Number(p) && high>=Number(p)) || (low==0 && high==0)){
								var b =a[i].getElementsByTagName("input");
								for(j=0; j<b.length;j++)
								{									
									if(b[j].name=="submit.addToCart"){ b[j].click();return}
            ...
            if(autocheckoutselect==="yes"){
				vv51 = setInterval(function () {
					if($("h2:contains('Il tuo carrello Amazon è vuoto.')").is(":visible")||$("h1:contains('Il tuo carrello Amazon è vuoto.')").is(":visible")){
						clearInterval(vv51);
						location.href=url;
					}
				},100); 
				...
				v78 = setInterval(function(){ 
					if($("#sw-atc-details-single-container").is(":visible")){
						try{
							if(soundAlert=="yes"){
								chrome.extension.sendMessage({action: "alert"}, function(response) {});
							}
               ...
               v12 = setInterval(function(){ 
				...
				v13 = setInterval(function(){ 
                    try{                    
                        if($("#shippingOptionFormId").is(":visible") || $("button:contains('Spedisci a questo indirizzo')").is(":visible")){
							clearInterval(v13);
							try{$("button:contains('Spedisci a questo indirizzo')").click()}catch(err){}
							try{$("[value='Continua']").eq(0).click();}catch(err){}
                        }                               
                   }catch(err){}
                },50);	
